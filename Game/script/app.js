...
class Board extends Game{
 ...
 Repetition(currpos){
  let positions = details.getPositions()
  let repetitionlength = positions.filter(pos => pos === currpos).length
  return repetitionlength >= 3
 }
 InsufficientMaterial(){
  let details = {white:[],black:[]}
  let [knights,bishops] = [0,0]
  let otherPieces = ["pawn","rook","queen"]
  let prevbishopColor = null
  let prevbishopSqColor = null
  for(let i = 0; i < board.getPieces().length; i++){
    let currentPiece = board.getPieces()[i]
    let type = currentPiece.getType()
    if(otherPieces.includes(type) || (knights > 1 || bishops > 2)) return false           
    let color = currentPiece.getColor() 
    let sqcolor = board.getSquare(currentPiece.getSquare())?.getColor()
    if(type == "bishop" && prevbishopColor == color) return false
    if(type == "bishop" && sqcolor == prevbishopSqColor) return true
    if(type == "bishop"){bishops++;prevbishopColor = color;prevbishopSqColor = sqcolor}     
    if(type == "knight") knights++  
    details[color].push(currentPiece)
  }
  let piecesNum = Object.values(details).flat().length
  if(piecesNum <= 3) return true
  return false
 }
 CheckMate(){
 ...
 Draw(currpos){
  let color = board.getTurnTeam()
  let currentKing = board.getPiece(`king_${color[0]}`)
  if(!currentKing.getisInCheck() && !board.hasLegal()){
    game.setis_Draw(true)
    game.resultActions("Draw!","No legal moves!")
  }
  if(game.gethalf_moves() == 50){
    game.setis_Draw(true)
    game.resultActions("Draw!","50-Move Rule!")
  }
  if(board.InsufficientMaterial()){
    game.setis_Draw(true)
    game.resultActions("Draw!","Insufficient Material!")
  }
  if(board.Repetition(currpos)){
    game.setis_Draw(true)
    game.resultActions("Draw!","Three-Fold Repetition!")
  }
 }
 hasLegal(){
  ...
  setCalcColor(){
    board.getSquares().forEach(square =>{ 
      ...
      let handleColors = (e) =>{ 
        ...
        let isDouble = newStamp - oldStamp <= 300
        e.stopImmediatePropagation()
        e.preventDefault()
        if(!isDouble) return     
        let target = e.target.classList.contains("pieces") ? e.target.parentNode : e.target
        let kingColor = board.getTurnTeam()[0]        
        let king = board.getPiece(`king_${kingColor}`)
        let kingSquare = king.getSquare()
        if(king.getisInCheck() && target.id == kingSquare) return    
      ...
      curr.addEventListener("contextmenu", () => board.getSquares().forEach(square => square.removeColor()))
    })
  }
  downloadGame(){
    let currentTime  = new Date()
    let date = [currentTime.getDate(),currentTime.getMonth()+1,currentTime.getFullYear(),currentTime.getHours(),currentTime.getMinutes()].map(num => num.toString().padStart(2, '0')).join(".").replace(11,"-")
    ...
    let text = Object.entries(content).map(([key, value]) => `${key}: ${value}`) .join("\n");
...
class Piece extends Board{
  ...
  #handleClick(e){
    ...
    let clickedPiece = board.clickedPiece ? board.clickedPiece.id : board.clickedPiece
    if(clickedPiece != e.target.id && (clickedPiece == null || board.clickedColor == piece.getColor())){
     board.clickedPiece = e.target    
     board.clickedColor = piece.getColor()    
    }else{   
      let dragPiece = board.getPiece(clickedPiece)
      let dragpieceelem = document.getElementById(clickedPiece)
      let targetsquareelem = document.getElementById(piece.getSquare())
      dragPiece.drop({drag:dragpieceelem,drop:targetsquareelem})
      board.clickedPiece = null   
    }  
    if(board.clickedPiece != null) piece.drag(e.target)  
  }
  disableLeftClick(){
  ...
  drag(piece,additionaMoves = []){
    ...
    let opponentsKing = board.getPiece(`king_${opponentsColor}`)
    let opponentsKingStatus = opponentsKing.getisInCheck()
    if(opponentsKingStatus) board.getSquare(opponentsKing.getSquare()).applyColor("red")     
  ...
  drop(data,additionaMoves = []){
    ...
    board.Evaluation()
    let fen = board.getstrFen().split(" ").splice(0,3).join(" ")//fen for theree fold repetition 
    fen += enpassant.isEnPassant ? ` ${enpassant.square}` : " -"
    if(!castle[targetSquare.id]) details.addPosition(fen)   
    board.Draw(fen)    
    board.CheckMate()
  ...
  #update(){
    ...
    static handleState(){
      this.handlevisibility = () =>{
        ...
        let timer = game.getTimer(board.getTurnTeam())
        state ? Timer.Offline(timer) : Timer.Online(timer)
      } 
      document.addEventListener("visibilitychange",this.handlevisibility)   
    }
    static balance(timer){
      let offtime = parseInt(timer.online / 1000 - timer.offline / 1000) - 1
      let availTime = timer.time - offtime > 0
      timer.time = availTime ? timer.time -= offtime : 1
      timer.start()
    }
    End(time = 0){
...
class Details{
  #movesNotation = []
  #positions = ["rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq -"]
  ...
  NotateMove(castle,type,startsquare,capture,targetSquare,promotion = {isPromotion:false}){
    ...
    let isCastle = castle ? castle.canCastle : false
    if(isCastle){notation = castle.move[0] == "g" ? "O-O" : "O-O-O";this.#removelastMove();return notation}
