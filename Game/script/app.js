...
class Board extends Game{
 ...
 updateFen(startY,destinationY,enpassant){
  ...
  squares.forEach(square =>{   
    ...
    if(sqobj.hasPiece()){
      let piecename = sqobj.getpieceName()
      ...
      console.log(newRow,spaces)    
    }else{spaces++}          
  })
  console.log(newRow,spaces)  
  board.setstrFen(board.getFen())
  console.log(turn,enpassant,halfMoves,fullMoves)
 }
 setFen(fen){
   this.#fen = fen
 }
...
class Piece extends Board{
  ...
  drop(data,additionaMoves = []){
    ...
    if(board.getTurnTeam() == "white") game.updatefull_moves()
    ...
    board.updateFen(startY,targetY,enpassantfen)
