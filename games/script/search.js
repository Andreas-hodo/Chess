...
async function connectEnemy(host,enemy,user){
  let roomid;
  ...
  const actions = {
    ...
    details: async (typedet, user, sectypeSet, doc) => {
      ...
      if (typedet === "counter") {
        ...
        details = { roomId: allusers, displayName: user.displayName, isnewUser: true, email: user.email };
      }
      return details;
    }
  };
               ...
               if(!roomElem){
                var newRoom = document.createElement("tr")
                   ...
                   newRoom.id = `${Enemyname}-${roomId}`
                    ...
                    newRoom.onclick = () =>{
                      connectEnemy(Enemyname,name,user)
                     }  
                } 
        }          
     })
