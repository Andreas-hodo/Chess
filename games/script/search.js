...
function createprofilepopup(data){
  ...
  <i class="fa-solid fa-x" id="closebtn" aria-hidden="true"></i>
      <h5 id="useremail">${data.email}</h5><h5>
      <div class="addImagebtncont"><input type="file" id="selectimagebtn" class="imagebtn"><i class="fa-solid fa-pencil" aria-hidden="true"></i></div>
      <div class="profilepicture" id="profpicture">  
          <img src="images/default-photoprofile.png" alt="photoProfile">
      </div>
      </h5><h5 id="disname">Hi,${data.displayName}!</h5>
    ...
    const {data,error} = await storage.storage.from("profile-photos").upload(file.name)
    if(error){
        alert("fail")
    }
    alert("succesfull")
