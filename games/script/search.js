...
function getUsersNum(numOfUsers){
     ...
     async function readMyUserDetails(collection,doc) {
      let details;
            if (!currentUser) {
        ...
        async function updateMyUserDetails(mode,typeSet,doc,typedet,user) {
          let actions = {
            ...
            details: async (typedet,user,typeset,doc)=>{
              let details;
              return await readMyUserDetails(typeset,doc).then(allusers =>{
            ...
            try {
              actions.details(typedet,user,typeSet,doc).then(Alldetails =>{
...
function connect_user(){
 ...
 firebase.auth().onAuthStateChanged((user) =>{
    if(user){
      currentUser = user
      let newUser = readMyUserDetails("usersDetails",user.uid).then(status =>{
             console.log(status)
      })
      ...
      playerid = user.uid
       ...
       if(newUser == "true"){
        updateMyUserDetails("update","UsersCounter","users","counter",user)
