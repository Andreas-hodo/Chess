...
function deleteacc(){    
            ...
            if (user) {
                // IMPORTANT: In a real app, you might need to re-authenticate the user first.
                // For example, if using email/password:
                const password = prompt("Please enter your password to confirm:");
                const credential = firebase.auth.EmailAuthProvider.credential(user.email, password);
                user.reauthenticateWithCredential(credential)
                    .then(() => { /* then call user.delete() */ })
                if (confirm("Are you sure you want to permanently delete your account? This cannot be undone.")) {
                    user.delete()
                        .then(() => {
                            console.log("User account deleted successfully!");
                            alert("Your account has been deleted. You will now be redirected.");
                            // Redirect to a login/signup page or home page
                        ...
                        .catch((error) => {
                            ...
                            if (error.code === 'auth/requires-recent-login') {
                                ...
                                // Prompt for re-authentication and then retry deletion
                            } else {
                ...
                window.location.href = "https://andreas-hodo.github.io/Chess/login/"; // Redirect to login
            }       
}
