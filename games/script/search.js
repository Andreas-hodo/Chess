...
async function cresupabaseacc(user,pass){
  try {
    ...
    if(user){
      ...
      name = user.displayName 
       if(newUser){
        ...
        await updateMyUserDetails("set","usersDetails","UsersCounter",user.uid,"roomId",user,"users").then(()=>{
        ...
        await readMyUserDetails("usersDetails",user.uid,null).then(status =>{
        roomid = status.roomId
        console.log("Room id not new: ",roomid)
        createprofilepopup({email:user.email,displayName:user.displayName,roomid})
      })
       }       
