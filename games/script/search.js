...
async function updateMyUserDetails(mode, typeSet, sectypeSet, doc, typedet, user, fieldkey) {
  ...
  try {
    ...
    alert("An error has occurred during the update process!");
    throw error;
  }
}
        // async function updateMyUserDetails(mode,typeSet,sectypeSet,doc,typedet,user,fieldkey) {
        ...
        //     }
        //     try {
        //       await actions.details(typedet,user,sectypeSet,fieldkey).then(Alldetails =>{
      ...
      name = user.displayName 
       if(newUser){
        ...
        await updateMyUserDetails("set","usersDetails","UsersCounter",user.uid,"roomId",user,"users").then((data)=>{
            ...
            return
            //readMyUserDetails("usersDetails",user.uid,null).then(status =>{
              roomid = status.roomId
              console.log("Room id new: ",roomid)
              console.log("Status: ",status)
              console.log("UID: ",user.uid)
              createprofilepopup({email:user.email,displayName:user.displayName,roomid})
            //})
        })
       }else{
        await readMyUserDetails("usersDetails",user.uid,null).then(status =>{
        roomid = status.roomId
        console.log("Room id not new: ",roomid,status)
        console.log("Status: ",status)
        console.log("UID: ",user.uid)
        createprofilepopup({email:user.email,displayName:user.displayName,roomid})
      })
