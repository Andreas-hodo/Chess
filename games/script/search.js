     ...
     async function readMyUserDetails(collection,doc,fieldkey) {
      let details;
            ...
            try {
                ...
                if (fieldkey) {
                details = docSnap.data()[fieldkey]
                //console.log("Details:",details,130)
                }else{
                  details = docSnap.data()
                  //console.log("Details:",details,133)
                }
                return details
            } catch (error) {
              console.log(error)
        ...
        async function updateMyUserDetails(mode,typeSet,doc,typedet,user,fieldkey) {
          let actions = {
            ...
            details: async (typedet,user,typeset,doc,fieldkey)=>{
              let details;
              return await readMyUserDetails(typeset,doc,fieldkey).then(allusers =>{
            ...
            try {
              actions.details(typedet,user,typeSet,doc,fieldkey).then(Alldetails =>{
...
function connect_user(){
 ...
 firebase.auth().onAuthStateChanged(async (user) =>{
    if(user){
      currentUser = user
      await readMyUserDetails("usersDetails",user.uid).then(status =>{
      ...
      name = user.displayName
       ...
       if(newUser){
        updateMyUserDetails("update","UsersCounter","users","counter",user,"users")
        updateMyUserDetails("set","usersDetails",user.uid,"roomId",user,"")
       }
