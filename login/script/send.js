...
function message(loggeduser,form,type,email,error = false,Errormessage = "",distype = ""){
           let currentUser = loggeduser.user ?  loggeduser.user : loggeduser
           let useremail = loggeduser.user ? loggeduser.user.email : email
  ...
  auth.onAuthStateChanged(async user => {
    if(user){  
      ...
      if(localStorage.getItem("emailSent") && localStorage.getItem("emailVerified") == ""){
        ...
        signup_btn.disabled = true
        signup_btn.style.opacity = "0.5"
        message(user,"signup","",user.email,false,"","verification")
        checkEmailVerified()
      }
    }   
