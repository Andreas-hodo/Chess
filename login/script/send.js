...
function createProfile(Url,displayName,email,auth){
  let isCreated = document.getElementById("userProfilepopup")
  if(!isCreated){
  let photoUrl = Url ? Url : "../games/images/default-photoprofile.png"
  let profile = document.createElement("div")
  profile.classList.add("profilepop")
  profile.id = "userProfilepopup"
  profile.title = "Click here to login automatically"
  let profileData = `
    <div class="photoCont"><img src="${photoUrl}" title="Profile Image" id="photoProfile"></div>
    <div class="UserName" title="Username">
    <p id="displayName">${displayName}</p>
    <p id="userEmail" title="Email">${email}</p>
    </div>
    <i id="removeBtn" class="fa-solid fa-x" aria-hidden="true" title="Remove account"></i>`
  profile.innerHTML = profileData
  document.body.appendChild(profile)
  let closeProf = document.getElementById("removeBtn")
  profile.addEventListener("click",()=>{
    window.location = "https://andreas-hodo.github.io/Chess/games/?new=false"
  })
  closeProf.addEventListener("click",(e)=>{
    e.stopImmediatePropagation()
    let profile = document.getElementById("userProfilepopup")    
    try{
      auth.signOut().then(()=>{
...
async function validation(){
  ...
  auth.onAuthStateChanged(async user => {
    if(user){ 
      let time = Date.parse(user.metadata.lastSignInTime)
      console.log(user)
      console.log(time)
      console.log(Date.now() - time)
      if(user.emailVerified && user.displayName && (Date.now() - time) >= 5000){
        createProfile(user.photoURL,user.displayName,user.email,auth)
      }            
