...
function message(loggeduser,form,type,email,error = false,message = "",distype = "",messageColor){
        ...
        e.target.onclick = () =>{}
        window.StorageBucket = {[`resend-${form}`]:{minutes:new Date().getMinutes() + 1,seconds:new Date().getSeconds()}}
        //sessionStorage.setItem(`resend-${form}`,60000)
        setTimeout(()=>{
              e.target.disabled = false
              e.target.style.opacity = "1"
              e.target.onclick = () =>{resendfunc(e)}
              window.StorageBucket[`resend-${form}`] = ""
...
async function validation(){
  ...
  if(!localStorage.getItem("AppInitialized")){
    ...
    window.StorageBucket = {["resend-login"]:"",["resend-signup"]:""}
  }
   const auth = firebase.auth()
   let index;
   const autochange = {
