...
auth.createUserWithEmailAndPassword(email,pass).then((loggeduser) =>{
   return loggeduser
    ...
    if (!loggeduser.emailVerified) { 
        ...
        container.innerHTML = `<p class="messages">An verifycation email was sent in</p>
<p class="messages" style="top:5%;">${loggeduser.user.email}</p>
<p class="messages resend">Didnt receive the Email?<a href="#" id="resendbtn">Resend Email</a></p>`
        signup.appendChild(container)
        resendBtn = document.getElementById("resendbtn")
        let resendfunc = (e) =>{
            e.target.disabled = true
            e.target.style.opacity = "0.5"
            loggeduser.user.sendEmailVerification({
          url: 'https://andreas-hodo.github.io/Chess/games/',
          handleCodeInApp: false,
        })
        e.target.disabled = true
