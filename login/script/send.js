...
async function getData(user,field) {
  const userDocRef = db.collection("usersDetails").doc(user.uid);
  if(field){
  const docSnapshot = await userDocRef.get()[field];
  return docSnapshot
  } 
  return null
}
...
async function validation(){
  ...
  auth.onAuthStateChanged(async user => {
    if(user){        
      ...
      getData(user,"emailSent").then(data =>{
         console.log("Email sent: ",data)
      })
      getData(user,"emailVerified").then(data =>{
  ...
  //console.log("Email Verified:",getData(user,"emailVerified"))
  //console.log("Display Name:",getData(user,"disname"))
